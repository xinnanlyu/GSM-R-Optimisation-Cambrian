function [BTSIndex] = GA3D(BTS, tx, Pmin, std, groupSize, iteration, mutProb, crossProb, display)
...
BTSIndex= floor(rand(groupSize,BTS)*trackLength)+1;

%% to make sure all genes layout are in asending order
BTSIndex=  sort(BTSIndex,2);


for iMain=1:iteration
    % Step 1 - mutation
    BTSIndex = Mutation(BTSIndex,mutProb,trackLength);
    
    % Step 2 - cross
    BTSIndex = Cross(BTSIndex,crossProb);
    
    % Step 3 - fitness
    fitness = CalcFitness(BTSIndex,BTSLocation, tx, std, Pmin, TRI);
    best=BTSIndex(fitness==max(fitness),:);
    ...
    % Step 4 - survivals
    t=toc;
    BTSIndex = Survival(BTSIndex,fitness,groupSize);
...
%% fitness function
function [result] = CalcFitness(BTSPosition,BTSLocationAll, tx, std, Pmin, TRI)
...
parfor nFitness=1:N
    ...
    Pr=Pt-PL;
    PrMax=max(Pr,[],2);
    % this shall be seperated to include the standard deviation calculation
    % or use STD_LUT to implement 
